...
export class ModalDetalleReservaComponent {
  ...
  constructor(public activeModal: NgbActiveModal, private dataSharingService: DataSharingService) {}
  receivedData: any;
  ...
  async cargarDatos(){
    this.departamento=this.selectedEvento['event']['_def']['extendedProps']['description']
    this.profesor=this.selectedEvento['event']['_def']['extendedProps']['professor']
    this.telefono=this.selectedEvento['event']['_def']['extendedProps']['telefono']
    ...
    this.horaMinutos = ("0" + horas).slice(-2) + ":" + ("0" + minutos).slice(-2);
  
    const horas2 = fechaFin.getHours();
    const minutos2 = fechaFin.getMinutes();
    this.horaMinutos2 = ("0" + horas2).slice(-2) + ":" + ("0" + minutos2).slice(-2);
